...
if command -v magick &> /dev/null; then
  ...
  USE_PILLOW=true
  echo "⚠️ ImageMagick 未安装，使用 Pillow 后备模式"
  echo "   如需更好效果，请安装: brew install imagemagick"
else
  echo "❌ 错误: 未安装 ImageMagick 或 Pillow"
...
if [ ! -s "$AI_IMG" ]; then
  ...
  echo ""
  echo "⚠️ 将使用渐变色占位图代替..."
  if [ "$USE_PILLOW" = true ]; then
    python3 -c "
from PIL import Image
img = Image.new('RGB', (${TOP_W}, ${TOP_H}), '#667eea')
img.save('$AI_IMG')
"
  else
...
ratio = max(${TOP_W}/img.width, ${TOP_H}/img.height)
img = img.resize((int(img.width*ratio), int(img.height*ratio)), Image.LANCZOS)
left = (img.width - ${TOP_W}) // 2
top = (img.height - ${TOP_H}) // 2
img = img.crop((left, top, left+${TOP_W}, top+${TOP_H}))
img.save('$AI_RESIZED')
"
else
...
for f in \
  ...
  "/System/Library/Fonts/Supplemental/STHeiti Medium.ttc"; do
  if [ -f "$f" ]; then
    FONT="$f"
    break
  fi
...
if [ -n "$FONT" ]; then
  ...
  echo "   Mac 中文字体通常在 /System/Library/Fonts/ 或 /Library/Fonts/"
  echo "   可安装阿里巴巴普惠体到 ~/Library/Fonts/"
fi
...
if [ "$USE_PILLOW" = true ]; then
  # ---- Pillow 模式：文字渲染 + 合成 + 拼接一步完成 ----
  PILLOW_PY="${TMP_DIR}/pillow_compose.py"
  cat > "$PILLOW_PY" << 'PYEOF'
import sys, os
from PIL import Image, ImageDraw, ImageFont

top_img_path = sys.argv[1]
output_path = sys.argv[2]
title = sys.argv[3]
bg_color = sys.argv[4]
text_color = sys.argv[5]
font_path = sys.argv[6] if len(sys.argv) > 6 and sys.argv[6] else ""
font_size = int(sys.argv[7]) if len(sys.argv) > 7 else 120

BOT_W, BOT_H = 1080, 720
PAD_LEFT, PAD_BOTTOM = 60, 60
TEXT_AREA_W = BOT_W - 120
TEXT_AREA_H = BOT_H - 120

# Load font
def load_font(size):
    if font_path and os.path.exists(font_path):
        return ImageFont.truetype(font_path, size)
    # Try common Mac system fonts
    for f in [
        "/System/Library/Fonts/STHeiti Medium.ttc",
        "/System/Library/Fonts/PingFang.ttc",
        "/Library/Fonts/Songti.ttc",
...
# Render text, auto-shrink if too large
...
while font_size > 40:
    ...
    if tw <= TEXT_AREA_W and th <= TEXT_AREA_H:
        break
    font_size -= 10

# Create bottom section
bottom = Image.new('RGB', (BOT_W, BOT_H), bg_color)
draw_bot = ImageDraw.Draw(bottom)
# Position: left-aligned, bottom-aligned
text_y = BOT_H - PAD_BOTTOM - th
draw_bot.multiline_text((PAD_LEFT, text_y), '\n'.join(lines),
                        fill=text_color, font=font, spacing=spacing)

# Load top image and combine
top = Image.open(top_img_path)
cover = Image.new('RGB', (1080, 1440))
cover.paste(top, (0, 0))
cover.paste(bottom, (0, 720))
cover.save(output_path)
print(f"{cover.width}x{cover.height}")
PYEOF

  echo "🔄 [2/3] + [3/3] Pillow 模式：文字渲染 + 拼接..."
  FINAL_SIZE=$(python3 "$PILLOW_PY" "$AI_RESIZED" "$OUTPUT" "$WRAPPED_TITLE" "$BG_COLOR" "$TEXT_COLOR" "$FONT" "$FONT_SIZE")
  echo "   最终字号: ${FONT_SIZE}px, 行数: ${LINE_COUNT}"
